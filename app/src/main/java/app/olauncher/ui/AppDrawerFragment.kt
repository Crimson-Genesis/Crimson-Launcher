...
class AppDrawerFragment : Fragment() {
    ...
    private fun setAlignment(alignment: Int) {
        ...
        val searchLayout = binding.searchBarContainer
        if (alignment == Gravity.START) {
            searchLayout?.gravity = Gravity.START
            searchTextView.gravity = Gravity.START
        } else {
            searchLayout?.gravity = Gravity.END
            searchTextView.gravity = Gravity.END
        }
    }
    ...
    private fun initAdapter() {
        adapter = AppDrawerAdapter(
            ...
            appHideListener = { appModel, _ ->
                ...
                if (newSet.isEmpty() && flag == Constants.FLAG_HIDDEN_APPS)
                    findNavController().popBackStack()
                if (prefs.firstHide) {
    ...
    private fun initObservers() {
        ...
        viewModel.homeAppAlignment.observe(viewLifecycleOwner, Observer {
            setAlignment(it)
        })
    }
    ...
    private fun checkMessageAndExit() {
        ...
        if (flag == Constants.FLAG_LAUNCH_APP)
            viewModel.checkForMessages.postValue(Unit)
    }
